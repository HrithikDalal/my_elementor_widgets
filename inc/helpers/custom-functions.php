<?php
/**
 * Hrithik features custom functions.
 *
 ...
 */
function hrithik_get_cache_key( $unique = '' ) {

	$cache_key = 'hrithik_cache_';
 ...
 */
function hrithik_get_cached_posts( $args ) {
	...
	$cache_key = hrithik_get_cache_key( $args );

	$cache  = new \Hrithik\Features\Inc\Cache( $cache_key );
 ...
 */
function hrithik_get_template_content( $slug, $vars = [] ) {
 ...
 */
function hrithik_features_template( $template, $variables = [], $echo = false ) {

	$template_file = sprintf( '%1$s/templates/%2$s.php', HRITHIK_FEATURES_PATH, $template );
 ...
 */
function hrithik_get_data( $slug, $default = [] ) {

	$data_file = sprintf( HRITHIK_FEATURES_PATH . '/inc/data/%s.php', $slug );
 ...
 */
function hrithik_is_production() {
 ...
 */
function hrithik_is_mobile( $kind = 'any', $return_matched_agent = false ) {
